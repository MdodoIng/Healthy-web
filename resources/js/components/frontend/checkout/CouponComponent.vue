    ...
    <div id="coupon-modal" class="fixed inset-0 z-50 p-3 w-screen h-screen overflow-y-auto bg-black/50 transition-all duration-300 opacity-0 invisible">
        <div class="w-full rounded-xl mx-auto bg-white transition-all duration-300 max-w-[360px]">
            <div class="flex items-center justify-between gap-2 py-4 px-4 border-b border-slate-100"><h3
                ...
                <button @click.prevent="hideTarget('coupon-modal', 'modal-active')" type="button" class="lab-line-circle-cross text-lg text-[#FB4E4E]"></button>
            </div>
            <form @submit.prevent="couponChecking" class="w-full flex items-center px-4 mt-4">
                <input :class="error ? 'invalid' : ''" type="text" v-model="code" class="h-11 w-full px-3 rounded-tl-lg rounded-bl-lg border border-r-0 border-[#D9DBE9] mb-5">
                <button type="submit" class="h-11 px-4 leading-11 rounded-tr-lg rounded-br-lg capitalize font-semibold text-white bg-clr mb-5">
                    {{ $t('button.apply') }}
                </button>
            </form>
