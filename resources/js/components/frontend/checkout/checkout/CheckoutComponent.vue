<template>
    ...
    <div class="row">
        <div class="col-12 lg:col-8">
            <div class="flex items-center radius-10 w-fit mb-6 text-focus bg-[#b2f3e2]">
                <div class="relative cursor-pointer">
                    <input @change="changeOrderType(orderTypeEnum.DELIVERY)" id="checkout-delivery"
                           ...
                           type="radio">
                    <label class="py-1.5 px-3.5 radius-10 text-sm font-semibold capitalize transition cursor-pointer"
            ...
            <div v-if="orderType === orderTypeEnum.PICK_UP" class="mb-6 rounded-2xl shadow-card border-clr">
                <h4 class="font-bold capitalize p-4 border-b border-gray-100">{{ $t('label.store_location') }}</h4>

                <div v-if="outlets.length > 0" v-for="outlet in outlets" class="px-4 pt-4 pb-4">
            ...
            <div class="max-lg:hidden flex items-center justify-between gap-5 mt-10">
                <router-link :to="{ name: 'frontend.checkout.cartList' }"
                             class="field-button w-fit font-semibold tracking-wide normal-case text-secondary bg-[#aef5e7]">
                    {{ $t('button.back_to_cart') }}
                </router-link>

                <button @click.prevent="selectAddress"
                        class="field-button w-fit font-semibold tracking-wide normal-case bg-save">
