<template>
    ...
    <div class="flex items-center gap-4 mb-7">
        <button @click.prevent="$router.back()" class="text-xl font-bold text-primary"></button>
        <!-- <h2 class="capitalize text-xl font-bold text-primary">{{ $t('label.order_details') }}</h2>-->
    </div>

    <div class="rounded-2xl shadow-card p-4 sm:p-6 mb-6 bg-white border border-[#D9DBE9]">
        <div class="text-center mb-10">
            ...
            <button v-if="order.status === enums.orderStatusEnum.CANCELED" type="button"
                class="flex items-center justify-center gap-2 py-3 sm:py-4 px-7 sm:px-10 mx-auto mt-6 rounded-2xl border border-danger text-[#FB4E4E] bg-white transition-all duration-500 hover:bg-[#FB4E4E] hover:text-white">
            ...
            <div class="col-12 md:col-7">
                <div class="rounded-2xl border border-gray-100">
                    ...
                    <div class="flex items-center justify-between py-3 px-4">
                        <span class="capitalize font-bold total-txx">{{ $t('label.total') }}</span>
                        <span class="capitalize font-bold total-txx">{{ order.total_currency_price }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div v-if="order.status !== enums.orderStatusEnum.CANCELED" class="flex flex-wrap gap-6 mobile:mb-20">
        ...
        <button v-if="order.status === enums.orderStatusEnum.PENDING" @click="changeStatus(enums.orderStatusEnum.CANCELED)"
            type="button"
            class="px-6 py-3 capitalize radius-10 whitespace-nowrap text-center font-semibold border border-danger text-danger bg-white">
    ...
    <div id="payment-modal" :class="Object.keys(route.query).length > 0 && route.query.status === 'success' && cart.length > 0 ? 'modal-active' : ''" class=" fixed inset-0 z-50 p-3 w-screen h-screen overflow-y-auto bg-black/50 transition-all duration-300 opacity-0 invisible">
        <div class="w-full rounded-xl mx-auto bg-white transition-all duration-300 max-w-[360px]">
            <div class="px-4 py-5 relative">
                ...
                <button type="button" @click.prevent="reset" class="field-button font-semibold normal-case bg-primary text-black">{{
