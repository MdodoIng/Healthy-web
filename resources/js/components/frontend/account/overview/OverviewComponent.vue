<template>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        <div class="p-3 rounded-2xl shadow-xs bg-white border border-gray-100">
            <i
    ...
    <div class="flex items-center justify-between mb-5">
        ...
        <RouterLink :to="{ name: 'frontend.account.orderHistory' }"
            class="py-1 px-4 text-sm sm:py-2 sm:px-5 radius-10 capitalize sm:text-base font-semibold whitespace-nowrap bg-primary text-black transition-all duration-300 hover:bg-primary hover:text-white">
            {{ $t('label.show_full_history') }}
        </RouterLink>
    </div>
    <div class="rounded-2xl shadow-card bg-white mobile:mb-20 border border-[#D9DBE9]">
            ...
            <table class="w-full text-left text-sm capitalize">
                ...
                <tbody class="font-medium" v-if="orders.length > 0">
                    <tr v-for="order in orders" :key="order">
                        ...
                        <td class="p-4 border-t border-gray-100">
                            <span class="ext-xs px-2 py-1 radius-10"
